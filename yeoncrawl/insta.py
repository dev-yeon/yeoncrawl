...
def insta_soup(request):
    ...
    # 해시태그 검색
    hashtag = metadata.HASH_TAG
    driver.get(f"{metadata.CONTENT_URL}{hashtag}/")
    time.sleep(60)
    ...
    items = item_soup.select(metadata.FAV_INSTAITEMS9_CSS)
...
def hayatteru_9():
    for j in range(1, 4):
        for k in range(1, 4):
            ...
            i_button = insta_utils.driver().find_element(By.XPATH, i_path)
            i_button.click()
            time.sleep(3)
            haya_item_html = insta_utils.driver().page_source
            ...
            while True:
                try:
                    ...
                    with open(img_name, "wb") as f:
                        f.write(insta_utils.driver(requests).get(img_url).content)
